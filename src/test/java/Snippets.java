...
public class Snippets {
    ...
    void browser_command_examples() {
        ...
        executeJavaScript("sessionStorage.clear();"); // no Selenide command for this yet, delete session Storage

        //set up cookie, after we have to do  Selenide.refresh();
        var cookie = new Cookie("foo", "bar");
        WebDriverRunner.getWebDriver().manage().addCookie(cookie);

        // for JavaScript alerts https://the-internet.herokuapp.com/javascript_alerts
        Selenide.confirm(); // OK in alert dialogs
        Selenide.dismiss(); // Cancel/close in alert dialogs

        // Windows and Frames
        Selenide.closeWindow(); // close active tab
        Selenide.closeWebDriver(); // close browser completely
        Selenide.switchTo().window("The Internet");//move to another windows

        //Frame https://the-internet.herokuapp.com/frames
        // "-" slow
        Selenide.switchTo().frame("new");// navigate to a frame, and we will be able to check an element in frame
    ...
    void actions_examples() {
        ...
        $("").setValue("text");// удалит, если было что-то написано
        $("").append("text");// добавит в текст конец
        $("").clear();// не всегда срабатывает в некоторых фреймворках
        $("").setValue(""); // вместо clear

        // эмулирование  работы кнопок на клавиатуре
    ...
    void conditions_examples() {
        ...
        $("").shouldHave(cssClass("red"),cssClass("blue"));//проверка двух класса, что оба есть
        ...
        $("").should(exist);//спрятанные значения

        // Warning! Checks only the "disabled" attribute! Will not work with many modern frameworks
        $("").shouldBe(disabled);// проверяют значение атрибута у элемента только
        $("").shouldBe(enabled);
    }

    void collections_examples() {

        $$("div"); // does nothing! коллекция -  строка ничего не ищет - надо добавить что делаем
        ...
        $$("div").first().click();// сводят из коллекции к одному элементу
        ...
        $$("").shouldHave(itemWithText("Gamma")); // only one text что в коллекции есть элемент, но не важно на каком месте
    ...
    void file_operation_examples() throws FileNotFoundException {
        ...
        // https://www.youtube.com/watch?v=x0KWgnjxsl4 Selenide 5.14.0 Download файлов
        // https://www.youtube.com/watch?v=txozPtQIqiE&t=1s Selenide 5.15.0 - Обзор релиза
